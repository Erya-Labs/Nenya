...
class Sha256NistVectorTest {
    ...
    private companion object {
        ...
        fun assertAllMatch(name: String, expectedCount: Int) {
            val vectors = NistSha256Vectors.messages(name)
    ...
    fun `every SHA256LongMsg vector hashes to NIST's digest`() {
        assertAllMatch(LONG_MSG, LONG_MSG_COUNT)
    }
    ...
    fun `the vendored NIST files match the checksums PROVENANCE records`() {
        for (name in listOf(SHORT_MSG, LONG_MSG, MONTE)) {
            val actual = hex(MessageDigest.getInstance("SHA-256").digest(NistSha256Vectors.file(name).readBytes()))
            assertEquals(NistSha256Vectors.recordedChecksum(name), actual, "checksum of $name")
        }
    }
